      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5">
        {visible.map((t, idx) => (
          <div key={idx} className="bg-white rounded-2xl border border-slate-100 shadow-sm p-5">
            ...
            <p className="text-navy/90 text-sm leading-relaxed mb-4">&ldquo;{t.text}&rdquo;</p>
            <div className="flex items-center gap-3">
              <div className={`w-9 h-9 rounded-full ${accentBg}/10 ${accentText} font-bold flex items-center justify-center text-sm shrink-0`}>
                {t.name.charAt(0)}
              </div>
              <div>
        ...
        <div className="flex items-center justify-center gap-1.5 mt-6">
          {Array.from({ length: pageCount }).map((_, idx) => (
            <button
              ...
              onClick={() => setPage(idx)}
              className={`w-2 h-2 rounded-full ${idx === page ? accentBg : "bg-slate-300"}`}
            />
          ))}
        </div>
      )}
...
/* ---------- Bottom CTA banner ---------- */
export function CtaBanner({ heading, sub, bg = "/images/cta-banner-bg.jpg", aspect = "2171 / 222", textInset = "pl-6 md:pl-10", showDirections = true, showMobileActions = true, theme = "navy" }) {
  return (
    <div className="mx-auto max-w-7xl px-4 py-6">
      <div className="relative rounded-2xl overflow-hidden hidden md:block">
        ...
        <div className={`absolute inset-0 flex flex-col md:flex-row items-center justify-between gap-4 pr-6 md:pr-10 ${textInset} text-white`}>
          <div className="text-center md:text-left shrink-0">
            <p className="font-display font-bold text-base md:text-lg leading-tight drop-shadow-md">{heading}</p>
            <p className="text-white/90 text-xs md:text-sm mt-0.5 drop-shadow-md">{sub}</p>
          </div>
          <div className="hidden md:flex items-stretch gap-6 lg:gap-8">
            ...
            <QuickAction href={`tel:${hospital.phoneTel}`} icon={<IconPhone />} label="Call Now" divider />
            {showDirections && (
              <QuickAction
      ...
      {/* Mobile — solid-color card, no background photo needed */}
      <div className={`rounded-2xl overflow-hidden md:hidden p-4 flex items-center gap-3.5 ${theme === "pink" ? "bg-gradient-to-br from-pink-light to-white border border-pink-light" : "bg-[#002062]"}`}>
        <span className={`w-12 h-12 rounded-full flex items-center justify-center shrink-0 ${theme === "pink" ? "bg-pink/15 border-2 border-pink/30 text-pink" : "bg-teal/25 border-2 border-teal/50 text-white"}`}>
          <IconPhone size={18} />
        </span>
        <div className="flex-1 min-w-0">
          <p className={`font-display font-bold text-sm leading-tight ${theme === "pink" ? "text-maroon" : "text-white"}`}>{heading}</p>
          <p className={`text-[11px] mt-0.5 leading-snug ${theme === "pink" ? "text-navy/70" : "text-white/75"}`}>{sub}</p>
        </div>
        <a href={`tel:${hospital.phoneTel}`} className={`shrink-0 text-xs font-semibold rounded-lg px-3 py-2.5 flex items-center gap-1.5 ${theme === "pink" ? "bg-pink hover:bg-maroon text-white" : "bg-white text-navy"}`}>
          <IconPhone size={13} /> Call Us
        </a>
      </div>
      {/* Mobile action row — the compact photo strip has no room for 4 two-line
          actions, so mobile gets simple icon-only buttons below it. Hidden entirely
          when the page already relies on the global sticky bar for actions. */}
      {showMobileActions && (
...
export function IconPhone({ size = 16 }) { return (<svg width={size} height={size} viewBox="0 0 24 24" fill="currentColor"><path d="M6.6 10.8a15 15 0 006.6 6.6l2.2-2.2c.3-.3.7-.4 1-.2 1.1.4 2.3.6 3.6.6.6 0 1 .4 1 1V20c0 .6-.4 1-1 1C10.6 21 3 13.4 3 4c0-.6.4-1 1-1h3.4c.6 0 1 .4 1 1 0 1.3.2 2.5.6 3.6.1.4 0 .8-.2 1z"/></svg>); }
...
export function IconMail({ size = 16 }) { return (<svg width={size} height={size} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><rect x="3" y="5" width="18" height="14" rx="2" /><path d="M3 7l9 6 9-6" /></svg>); }
...
/* ---------- Horizontal scroller with prev/next arrow buttons ---------- */
export function ScrollerWithArrows({ children, accent = "teal" }) {
  ...
  return (
    <div className="relative">
      <button
        ...
        onClick={() => scroll(-1)}
        className={`hidden md:flex absolute -left-4 top-1/2 -translate-y-1/2 z-10 w-9 h-9 rounded-full bg-white shadow items-center justify-center ${accentText}`}
      >
        <IconChevronLeft />
      </button>
      <div
      ...
      <button
        ...
        onClick={() => scroll(1)}
        className={`hidden md:flex absolute -right-4 top-1/2 -translate-y-1/2 z-10 w-9 h-9 rounded-full bg-white shadow items-center justify-center ${accentText}`}
      >
        <IconChevronRight />
      </button>
    </div>
...
export function TestimonialGrid({ items }) {
  ...
  const DOT_COUNT = 4;
  const groupSize = Math.max(1, Math.ceil(items.length / DOT_COUNT));
  const activeDot = Math.min(DOT_COUNT - 1, Math.floor(active / groupSize));
  ...
  return (
    <div className="relative">
      ...
      >
        {items.map((t, i) => (
          <div key={i} className="snap-start shrink-0 w-[85%] sm:w-[46%] lg:w-[24%] bg-white rounded-2xl border border-slate-100 shadow-sm p-5 flex flex-col h-56">
            ...
            <p className="text-sm text-navy/90 leading-relaxed mb-4 line-clamp-4">{t.text}</p>
      ...
      <div className="flex items-center justify-center gap-3 mt-4">
        ...
        <div className="flex items-center gap-2">
          {Array.from({ length: DOT_COUNT }).map((_, dotIdx) => (
            <button
              key={dotIdx}
              type="button"
              aria-label={`Go to testimonial group ${dotIdx + 1}`}
              onClick={() => scrollToIndex(dotIdx * groupSize)}
              className={`rounded-full transition-all ${dotIdx === activeDot ? "w-6 h-2 bg-navy" : "w-2 h-2 bg-slate-300"}`}
            />
          ))}
        </div>
        <button
